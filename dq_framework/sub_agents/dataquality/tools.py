...
from dq_framework.utils.utils import get_env_var
...
def get_bigquery_schema(dataset_id, client=None, project_id=None):
    ...
    Args:
        dataset_id (list): The IDs of the BigQuery dataset (e.g., ['my_dataset', 'my_other_dataset']).
...
def run_bigquery_validation(
    ...
    def cleanup_sql(sql_string):
        ...
        if "limit" not in sql_string.lower() and "insert" not in sql_string.lower():
  ...
  #  if re.search(
   #     r"(?i)(update|delete|drop|create|alter|truncate|merge)", sql_string
    #):
            ...
            rows = [
                {
                    key: (
                        ...
                        else value.strftime("%Y-%m-%d") if isinstance(value, datetime.date) # Then handle datetime.date
                        else value
                    )
...
def execute_bigquery_insert(
    sql_insert_statement: str,
    tool_context: ToolContext, # Keep for consistency, even if unused for now
) -> dict:
    """Executes a BigQuery INSERT statement.

    Args:
        sql_insert_statement (str): The SQL INSERT statement to execute.
        tool_context (ToolContext): The tool context (currently unused but kept for consistency).

    Returns:
        dict: A dictionary containing the execution status:
              - {"status": "success", "message": "Insert successful. Rows affected: <int>"}
              - {"status": "error", "message": "Error executing insert: <error_details>"}
              - {"status": "error", "message": "Invalid operation: Only INSERT statements are allowed."}
    """
    logging.info("Attempting to execute BigQuery INSERT statement.")
    # Basic check to ensure it looks like an INSERT statement
    # This is a safeguard, not foolproof security.
    cleaned_sql = sql_insert_statement.strip()
    if not cleaned_sql.upper().startswith("INSERT"):
        logging.error("Provided SQL is not an INSERT statement: %s", sql_insert_statement)
        return {
            "status": "error",
            "message": "Invalid operation: Only INSERT statements are allowed by this tool.",
        }

    # Optional: Add more robust cleanup if needed, similar to validation function
    # cleaned_sql = cleanup_sql(cleaned_sql) # If you define cleanup_sql globally or pass it

    logging.info("Executing BigQuery INSERT: %s", cleaned_sql)
    final_result = {"status": None, "message": None}
    client = get_bq_client()

    try:
        # Execute the query (not a dry run)
        query_job = client.query(cleaned_sql)
        query_job.result()  # Wait for the job to complete and raise exceptions on errors

        rows_affected = query_job.num_dml_affected_rows
        final_result["status"] = "success"
        if rows_affected is not None:
             final_result["message"] = f"Insert statement executed successfully. Rows affected: {rows_affected}."
             final_result["rows_affected"] = rows_affected # Optionally return the count
        else:
             # This might happen for certain types of inserts or if info isn't available
             final_result["message"] = "Insert statement executed successfully. Number of affected rows not available."


        logging.info("BigQuery INSERT successful. Job ID: %s, Rows Affected: %s", query_job.job_id, rows_affected)

    except Exception as e:
        logging.error("Error executing BigQuery INSERT statement: %s", e, exc_info=True)
        final_result["status"] = "error"
        final_result["message"] = f"Error executing insert: {e}"

    print("\n execute_bigquery_insert final_result: \n", final_result)
    return final_result